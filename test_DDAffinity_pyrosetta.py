...
# Fix for chain ordering issue without modifying original files
def _get_structure_fixed(pdb_path, chains, pdbcode, flag):
    parser = PDBParser(QUIET=True)
    model = parser.get_structure(None, pdb_path)[0]
    chains_ordered = [chain.id for chain in chains]
    data, seq_map = parse_biopython_structure(model, chains_ordered)
    _structures = {}
    _structures[flag + "_" +pdbcode] = (data, seq_map)
    return _structures

# Monkey patch the function in the original module
import rde.datasets.skempi_parallel as skempi_parallel
skempi_parallel._get_structure = _get_structure_fixed

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('config', type=str)
    parser.add_argument('--output_results', type=str, default='pyrosetta_results.csv')
    parser.add_argument('--output_metrics', type=str, default='pyrosetta_metrics.csv')
