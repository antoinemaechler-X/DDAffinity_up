...
from rde.utils.skempi_spr_mpnn import SkempiSPRDatasetManager
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', type=str, default='./logs_skempi_spr')
    parser.add_argument('--early_stoppingdir', type=str, default='./early_stopping_spr')
    ...
    dataset_mgr = SkempiSPRDatasetManager(
        config,
    ...
    try:
        ...
        if True:
            results = []
            # Saving 10-fold checkpoint: DDAffinity_SPR
            logger.info(f'Saving checkpoint: DDAffinity_SPR.pt')
            cv_mgr.save_state_dict(args, config)
            # Loading checkpoint: DDAffinity_SPR
            ckpt_path = os.path.join(early_stoppingdir, 'DDAffinity_SPR.pt')
